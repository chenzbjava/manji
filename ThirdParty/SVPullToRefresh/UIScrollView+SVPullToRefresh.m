...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    if(!showsPullToRefresh) {
        ...
        if (!self.pullToRefreshView.isObserving) {
            ...
            //改变上拉刷新文本的宽度 self.pullToRefreshView.frame
            self.pullToRefreshView.frame = CGRectMake(0, yOrigin, self.bounds.size.width*2, SVPullToRefreshViewHeight);
        }
    }
}
...
- (void)stopAnimating {
    ...
    switch (self.position) {
        case SVPullToRefreshPositionTop:{
            //          if(!self.wasTriggeredByUser){ //modify by zhuhao at 20131015
            [UIView animateWithDuration:0.3
                                  delay:0.3
